...
public class CrownValidatorImpl implements CrownValidator {
        ...
        if (request.getDetail().isEmpty()) {
            ...
            for (int i = 0; i < request.getDetail().size(); i++) {
                ...
                if (StringUtils.isBlank(request.getDetail().get(i).getCounselLastNm())) {
                    errors.add(
                            MessageFormat.format(
                                    "Details CounselLastNm at index {0} is not valid", i + 1));
                }

                if (StringUtils.isBlank(request.getDetail().get(i).getCounselFirstNm())) {
                    errors.add(
                            MessageFormat.format(
                                    "Details CounselFirstNm at index {0} is not valid", i + 1));
                }
            }
        }
        ...
        if (request.getCrownAssignment().isEmpty()) {
            ...
            for (int i = 0; i < request.getCrownAssignment().size(); i++) {
                ...
                if (StringUtils.isNoneEmpty(
                                request.getCrownAssignment().get(i).getAssigningPaasAgencyId())
                        && checkAgencyIdentifierTypeForErrors(
                                request.getCrownAssignment().get(i).getAssigningPaasAgencyId())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment AssigningPaasAgencyId at index {0} is not valid",
                                    i + 1));
                }

                if (StringUtils.isNoneEmpty(
                                request.getCrownAssignment().get(i).getAssigningPaasPartId())
                        && checkSystemIdTypeForErrors(
                                request.getCrownAssignment().get(i).getAssigningPaasPartId())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment AssigningPaasPartId at index {0} is not valid",
                                    i + 1));
                }

                if (StringUtils.isNoneEmpty(
                                request.getCrownAssignment().get(i).getAssigningPaasSeqNo())
                        && checkSystemSeqNoTypeForErrors(
                                request.getCrownAssignment().get(i).getAssigningPaasSeqNo())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment AssigningPaasSeqNo at index {0} is not valid",
                                    i + 1));
                }
                ...
                if (request.getCrownAssignment().get(i).getOperationModeCd() == null) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment OperationModeCd at index {0} is not valid",
                                    i + 1));
                }

                if (StringUtils.isNoneEmpty(request.getCrownAssignment().get(i).getPaasAgencyId())
                        && checkAgencyIdentifierTypeForErrors(
                                request.getCrownAssignment().get(i).getPaasAgencyId())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment PaasAgencyId at index {0} is not valid",
                                    i + 1));
                }

                if (StringUtils.isNoneEmpty(request.getCrownAssignment().get(i).getPaasPartId())
                        && checkSystemIdTypeForErrors(
                                request.getCrownAssignment().get(i).getPaasPartId())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment PaasPartId at index {0} is not valid", i + 1));
                }

                if (StringUtils.isNoneEmpty(request.getCrownAssignment().get(i).getPaasSeqNo())
                        && checkSystemSeqNoTypeForErrors(
                                request.getCrownAssignment().get(i).getPaasSeqNo())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment PaasSeqNo at index {0} is not valid", i + 1));
                }

                if (StringUtils.isNoneEmpty(
                                request.getCrownAssignment().get(i).getWorkAssignmentCcn())
                        && checkConcurrencyControlTypeForErrors(
                                request.getCrownAssignment().get(i).getWorkAssignmentCcn())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment WorkAssignmentCcn at index {0} is not valid",
                                    i + 1));
                }

                if (StringUtils.isNoneEmpty(
                                request.getCrownAssignment().get(i).getWorkAssignmentId())
                        && checkSystemIdTypeForErrors(
                                request.getCrownAssignment().get(i).getWorkAssignmentId())) {
                    errors.add(
                            MessageFormat.format(
                                    "CrownAssignment WorkAssignmentId at index {0} is not valid",
                                    i + 1));
                }
            }
        }
    ...
    public List<String> validateSetCrownFileDetail(SetCrownFileDetailRequest request) {
        ...
        if (StringUtils.isBlank(request.getMdocCcn())
                || checkConcurrencyControlTypeForErrors(request.getMdocCcn())) {
