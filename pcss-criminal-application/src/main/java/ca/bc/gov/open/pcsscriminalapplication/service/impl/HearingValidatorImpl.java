...
public class HearingValidatorImpl implements HearingValidator {
    ...
    public List<String> validateSetHearingRestrictionCriminal(
        ...
        if (StringUtils.isNoneEmpty(request.getHearingRestrictionId())
                && checkSystemIdTypeForErrors(request.getHearingRestrictionId())) {
            errors.add("HearingRestrictionId is not valid");
        }

        if (StringUtils.isNoneEmpty(request.getAdjudicatorPartId())
                && checkSystemIdTypeForErrors(request.getAdjudicatorPartId())) {
            errors.add("AdjudicatorPartId is not valid");
        }

        if (StringUtils.isNoneEmpty(request.getJustinNo())
                && checkJustinNoTypeForErrors(request.getJustinNo())) {
            errors.add("JustinNo is not valid");
        }

        if (StringUtils.isNoneEmpty(request.getPartId())
                && checkSystemIdTypeForErrors(request.getPartId())) {
            errors.add("PartId is not valid");
        }

        if (StringUtils.isNoneEmpty(request.getProfSeqNo())
                && checkSystemSeqNoTypeForErrors(request.getProfSeqNo())) {
            errors.add("ProfSeqNo is not valid");
        }

        if (StringUtils.isNoneEmpty(request.getHearingRestrictionCcn())
                && checkConcurrencyControlTypeForErrors(request.getHearingRestrictionCcn())) {
